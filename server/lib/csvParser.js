/**
 * Parses the IG Wealth Management net worth statement CSV data
...
function isIndented(rawDescription) {
  ...
  // Check for leading spaces, non-breaking spaces, or tabs
...
export function parseCSVData(rows) {
  ...
  // Find the header row (contains "Description" and owner names)
  ...
  if (headerRowIndex === -1) {
    // Fallback: assume row 1 is headers (row 0 is title)
    headerRowIndex = 1;
    headers = rows[1] ? rows[1].map((h) => normalizeString(String(h || ''))) : [];
  }

  // Extract owner columns (between "Valuation date" and "Total")
  const descIdx = 0;
  ...
  const pctIdx = headers.findIndex((h) => h.toLowerCase().includes('% total') || h.toLowerCase().includes('%total'));
  ...
  for (let r = headerRowIndex + 1; r < rows.length; r++) {
    ...
    // Check if this is the footer row (last meaningful row with client info)
    // Footer typically contains advisor/client name info
    if (r >= rows.length - 3) {
    ...
    // Determine if this is a holding (indented) or an account
    const indented = isIndented(rawDescription);

    if (indented && currentAccount) {
      // This is a holding under the current account
      ...
      // Extract fund code from description
      const fundCodeMatch = description.match(
        /\b([A-Z]{2,5}\d{2,5})\b|^([A-Z]{2,6}\s?\d{3,5})/i
      );
      ...
      const accountType = currentSection
        ? SECTION_TO_ACCOUNT_TYPE[currentSection] || 'Other'
        : 'Other';
      const defaultAssetClass = currentSection
        ? SECTION_TO_DEFAULT_CLASS[currentSection] || 'Other'
        : 'Other';

      // Determine asset class overrides
      let assetClass = defaultAssetClass;
      const descLower = description.toLowerCase();
      if (
        descLower.includes('tax holding') ||
        descLower.includes('tax reserve') ||
        descLower.includes('cash') ||
        descLower.includes('hisa') ||
  ...
  // Try to extract footer from the last few rows if not found
