/**
 * Classifies assets and applies planning flags.
 */
...
export function classifyAndFlag(parsedData) {
  ...
  for (const group of Object.values(classGroups)) {
    ...
    if (group.percentOfTotal > 40) {
      ...
      planningFlags.push(
        `🔴 High concentration: ${group.name} at ${group.percentOfTotal.toFixed(1)}% of gross assets`
      );
    } else if (group.percentOfTotal > 30) {
      group.concentrationFlag = 'moderate';
      planningFlags.push(
        `🟡 Moderate concentration: ${group.name} at ${group.percentOfTotal.toFixed(1)}% of gross assets`
      );
    } else {
      group.concentrationFlag = 'none';
    }

    // Check individual assets for location flags
    for (const asset of group.assets) {
      asset.locationFlag = false;
      const cls = asset.assetClass?.toLowerCase() || '';
      if (
        (cls === 'cash' || cls === 'fixed income') &&
        asset.accountType === 'Non-registered'
      ) {
        asset.locationFlag = true;
        planningFlags.push(
          `⚠ Tax location: ${asset.description} — ${asset.assetClass} held in non-registered account`
        );
      }
    }
  }

  // Check single-entity concentration
  for (const asset of assets) {
    if (grossAssets > 0 && (asset.totalValue / grossAssets) * 100 > 40) {
      planningFlags.push(
        `🔴 Single-entity risk: ${asset.description} at ${((asset.totalValue / grossAssets) * 100).toFixed(1)}% of gross assets`
      );
    }
  }
